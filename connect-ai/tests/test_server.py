...
def test_connector_tool_settings_and_audit_rest(tmp_path):
    ...
    assert any(t["name"] == "browser_open" for t in connectors["browser"]["tools"])

    res = client.patch(
        "/v1/connectors/browser/tools", json={"enabled": {"browser_open": False}}
    ).json()
    ...
    assert browser_tools["browser_open"]["enabled"] is False
...
def test_artifacts_resolve_shared_outputs_tree(tmp_path, monkeypatch):
    """save_csv & friends write to <project>/outputs, not the session workspace —
    the rail must list those files and `artifact:outputs/...` chips must open them."""
    workspace = tmp_path / "ws"
    workspace.mkdir()
    outputs = tmp_path / "outputs"
    (outputs / "csv").mkdir(parents=True)
    (outputs / "csv" / "report.csv").write_text("a,b\n1,2\n", encoding="utf-8")
    monkeypatch.setenv("COWORKER_OUTPUT_DIR", str(outputs))

    client = _client(workspace, [])
    by_path = {a["path"]: a for a in client.get("/v1/sessions/unknown/artifacts").json()["artifacts"]}
    assert by_path["outputs/csv/report.csv"]["kind"] == "csv"

    # Both the prefixed chip path and the bare outputs-relative path resolve.
    for path in ("outputs/csv/report.csv", "csv/report.csv", str(outputs / "csv" / "report.csv")):
        read = client.get(
            "/v1/sessions/unknown/artifacts/read", params={"path": path}
        ).json()
        assert read["ok"] is True, path
        assert read["content"].startswith("a,b")

    missing = client.get(
        "/v1/sessions/unknown/artifacts/read", params={"path": "outputs/csv/nope.csv"}
    ).json()
