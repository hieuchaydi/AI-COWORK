...
def test_convert_assistant_tool_turn_maps_role_model():
    ...
    # No captured signature (this history never went through Gemini): the call carries the
...
def test_convert_reattaches_signatures_to_parts():
    ...
    # First call had no captured signature → skip-validation sentinel (not omitted).
    assert parts[1]["thought_signature"] == _SKIP_SIGNATURE
    assert parts[2]["function_call"]["name"] == "b"
    assert parts[2]["thought_signature"] == "Y3NpZw=="  # real signature preserved


def test_convert_unsigned_calls_get_skip_sentinel():
    # A tool turn produced by another model (quota failover) has no `_gemini` sidecar at
    # all. Every function_call must still carry a signature or Gemini 3 400s — each gets
    # the skip-validation sentinel.
    _, contents = convert_messages(
        [
            {"role": "user", "content": "go"},
            {
                ...
                "tool_calls": [
                    {"id": "call_0", "type": "function", "function": {"name": "a", "arguments": "{}"}},
                    {"id": "call_1", "type": "function", "function": {"name": "todo_write", "arguments": "{}"}},
                    {"id": "call_2", "type": "function", "function": {"name": "c", "arguments": "{}"}},
                ],
            },
        ]
    )
    calls = [p for p in contents[1]["parts"] if "function_call" in p]
    assert len(calls) == 3
    assert all(p["thought_signature"] == _SKIP_SIGNATURE for p in calls)
