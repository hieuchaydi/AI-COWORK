...
def test_engine_connector_tools_are_cowork_scoped(tmp_path):
    ...
    assert "browser_open" in cowork.registry.names()
    assert "browser_click" in cowork.registry.names()
    assert "browser_fill" in cowork.registry.names()
    ...
    assert cowork.registry.get("browser_open").metadata.requires_approval is False
    assert cowork.registry.get("browser_read").metadata.requires_approval is False
    assert cowork.registry.get("browser_click").metadata.requires_approval is True
    assert cowork.registry.get("browser_fill").metadata.requires_approval is True
...
def test_connector_list_descriptors(tmp_path):
    ...
    assert any(
        t["name"] == "browser_open" and t["requires_approval"]
