...
def test_output_budget_only_fills_a_gap_and_only_when_opted_in():
    """Workers AI caps output at 256 when max_tokens is absent — a reasoning model spends
    all of it thinking and returns an empty message with finish_reason=length."""
    msgs = [{"role": "user", "content": "hi"}]

    plain = _FakeClient(_response(content="ok"))
    OpenAIProvider(client=plain).complete(model="gpt-5.5", messages=msgs)
    assert "max_tokens" not in plain.chat.completions.calls[0]  # OpenAI needs no help

    cf = _FakeClient(_response(content="ok"))
    OpenAIProvider(client=cf, default_max_tokens=8192).complete(model="@cf/x", messages=msgs)
    assert cf.chat.completions.calls[0]["max_tokens"] == 8192

    # An explicit caller setting always wins, in either spelling.
    explicit = _FakeClient(_response(content="ok"))
    p = OpenAIProvider(client=explicit, default_max_tokens=8192)
    p.complete(model="@cf/x", messages=msgs, max_tokens=64)
    p.complete(model="@cf/x", messages=msgs, max_completion_tokens=99)
    assert explicit.chat.completions.calls[0]["max_tokens"] == 64
...
def _fat_tool_list():
    """~200-tool payload shaped like the real runtime: built-ins + bulky telegram +
    utility bridges. Exceeds Groq's 128 cap."""
    tools = [_tool(f"builtin_{i}") for i in range(40)]
    tools += [_tool(f"mcp__telegram_bot__op{i}") for i in range(91)]
    tools += [_tool(f"mcp__skills__op{i}") for i in range(28)]
    tools += [_tool(f"mcp__filesystem__op{i}") for i in range(45)]
    return tools


def test_groq_trims_tools_to_cap_keeping_important():
    client = _FakeClient(_response(content="ok"))
...
def test_uncapped_provider_sends_all_tools():
    client = _FakeClient(_response(content="ok"))
    provider = OpenAIProvider(client=client, base_url="https://api.cloudflare.com/x")
    tools = _fat_tool_list()
    provider.complete(model="@cf/openai/gpt-oss-120b", messages=[], tools=tools)
    assert len(client.chat.completions.calls[0]["tools"]) == len(tools)
