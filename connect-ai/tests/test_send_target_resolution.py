...
def test_browser_reads_are_free_interactions_gate():
    from coworker.tools.browser import make_browser_tools

    tools = {t.__name__: t for t in make_browser_tools()}
    assert tools["browser_read"].__aisuite_tool_metadata__.requires_approval is False
    assert tools["browser_open"].__aisuite_tool_metadata__.requires_approval is False
    assert tools["browser_click"].__aisuite_tool_metadata__.requires_approval is True
    assert tools["browser_fill"].__aisuite_tool_metadata__.requires_approval is True
