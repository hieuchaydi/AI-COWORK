...
// is the tools page — the Google pair connects here via the local 1-click login; other
// tools are added later from the Connectors page. Entered here via the REPLAY path
...
test("key form: Test verifies, saves, and returns to the gallery with the ✓", async ({
  ...
  await expect(page.getByRole("heading", { name: "Welcome to Workspace" })).toBeVisible();
...
test("tools page: Google 1-click login flips the combined row + band; other tools defer", async ({
  page,
}) => {
  await openOnboarding(page);
  await page.getByTestId("ob-continue").click();
  ...
  // Pre-connect: benefit rows are there (non-Google defer to the Connectors page), the
  // combined Google row + band offer the local sign-in, Next is always available.
  ...
  await expect(page.getByTestId("ob-tool-outlook")).toContainText("From the Connectors page");
  await expect(page.getByTestId("ob-tool-google")).toContainText("Gmail & Google Calendar");
  await expect(page.getByTestId("ob-google-band-connect")).toContainText("Sign in with Google");

  // The 1-click login completes out-of-band (mock consent lands instantly); the combined
  // row and the band both flip to the connected state.
  await page.getByTestId("ob-google-connect").click();
  await expect(page.getByTestId("ob-tools-google-done")).toBeVisible({ timeout: 10_000 });
  await expect(page.getByTestId("ob-tools-google-done")).toContainText("Google is connected");
  await expect(page.getByTestId("ob-tool-google")).toContainText("✓ Connected");
...
test("tools page continues without connecting; Start working lands in a session with the panel open", async ({
  page,
}) => {
  await openOnboarding(page);
  await page.getByTestId("ob-continue").click();
