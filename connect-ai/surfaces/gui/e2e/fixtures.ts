...
const PERSONAS = {
  personas: [
    { id: "cowork", name: "AI cowork", icon: "cowork", tagline: "Produce a deliverable — research, analysis, scripts", needs_workspace: true, builtin: true, family: "knowledge", workspace: "deliverable", tools: ["files", "search"], enabled: true, surfaced: true, default: true },
...
const PINNED_SESSION = {
  ...
  workspace: "/Users/test/AI cowork/launch-note",
  agent: "cowork",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const OPS_SESSION = {
  session_id: "ops-1",
  title: "Ops triage",
  workspace: "/Users/test/AI cowork/ops-triage",
  agent: "ops",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const PRIMARY_ROOT = { path: "/Users/test/AI cowork/launch-note", writable: true, label: "scratch", primary: true, exists: true };
...
export async function mockApi(page: import("@playwright/test").Page) {
  ...
  // One Google login (helper :8766 auth-start) = the next account lands on BOTH
  // gmail and calendar, mirroring google_auth.py's push to the connector trio.
  const connectNextGoogleAccount = () => {
    ...
    const gEmail = GCAL_NEXT[gcalState.accounts.length] || `acct${gcalState.accounts.length}@x.com`;
    gcalState.accounts.push({
      email: gEmail, default: gcalState.accounts.length === 0, managed: true,
  ...
  // Local Google helper (launcher :8766): the GUI's "Sign in with Google" calls
  // auth-start; the consent "completes in the browser" instantly here — the next
  // account lands and the pages' polls pick it up.
  await page.route("**/google/auth-start*", async (route) => {
    connectNextGoogleAccount();
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({ auth_url: "https://accounts.google.com/o/oauth2/v2/auth?mock", opened_browser: true }),
    });
  });
  // The Connectors list's Google card reads this: who is signed in, and which
  // Google services that ONE login is driving (google_auth.py's view).
  await page.route("**/google/setup-state*", async (route) => {
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({
        has_client: true,
        accounts: gmailState.accounts.map((a, i) => ({
          name: a.email,
          email: a.email,
        ...
        services: {
          gmail: {
            connected: gmailState.accounts.length > 0,
            accounts: gmailState.accounts.map((a) => a.email),
            title: "Gmail",
          },
          google_calendar: {
            connected: gcalState.accounts.length > 0,
            accounts: gcalState.accounts.map((a) => a.email),
  ...
  await page.route("**/v1/**", async (route) => {
    ...
    if (p.endsWith("/v1/personas/install") && m === "POST") {
      const b = req.postDataJSON();
      if (b.gallery_slug) {
        return json({ ok: true, consent: [{ id: b.gallery_slug }], personas });
      }
    ...
    // Manual token connect (the add-modal's ConnectSetup): notion adds the next
    // workspace, hubspot the next portal (read tier) — cloud one-click is gone.
    if (p.endsWith("/v1/connectors/slack/connect") && m === "POST") {
    ...
    if (p.endsWith("/v1/connectors/github/connect") && m === "POST") {
    ...
    if (p.endsWith("/v1/connectors/notion/connect") && m === "POST") {
    ...
    if (p.endsWith("/v1/connectors/hubspot/connect") && m === "POST") {
      const next = HUBSPOT_NEXT[hubspotState.portals.length] || {
        hub_id: `9${hubspotState.portals.length}`, name: "extra", sandbox: false,
      };
      hubspotState.portals.push({ ...next, default: hubspotState.portals.length === 0, managed: false, access: "read" });
      return json({ ok: true });
    }
    ...
    if (p.endsWith("/v1/cloud/gallery")) {
      return json({ ok: true, personas: GALLERY_PERSONAS });
    }
