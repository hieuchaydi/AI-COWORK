...
// (Socket-Mode bot+app token — cloud one-click removed), per-workspace disconnect
// (stop-relaying-only), and the manual Socket-Mode card.
...
test("Add workspace opens the token modal; connecting installs a new workspace", async ({
  page,
}) => {
  await openSlackPage(page);
  await page.getByTestId("add-workspace-btn").click();
  const modal = page.getByTestId("add-connection-modal");
  // Socket-Mode bot + app tokens are the only connect path now — no cloud one-click.
  ...
  await modal.getByPlaceholder("Bot token · xoxb-…").fill("xoxb-test");
  await modal.getByPlaceholder("App token · xapp-…").fill("xapp-test");
  await modal.getByRole("button", { name: "Connect", exact: true }).click();
  // the mock connects instantly; the page's poll shows the new workspace
