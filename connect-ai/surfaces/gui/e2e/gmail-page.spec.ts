...
// Default badge, per-account disconnect, 1-click Google login add, and the
// "Never show agents" filter lists.
...
async function connectFirstAccount(page) {
  ...
  // gmail starts disconnected → Available row → modal → Sign in with Google
  // (the mock consent lands instantly; the modal closes itself on the flip)
  await page.getByTestId("connector-gmail").getByRole("button", { name: "Connect", exact: true }).click();
  await page.getByTestId("modal-google-connect").click();
