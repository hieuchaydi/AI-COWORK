...
const apiToken = (): string =>
  ...
  (import.meta as any).env?.VITE_CONNECT_AI_API_TOKEN ||
  (import.meta as any).env?.VITE_COWORKER_API_TOKEN ||  // legacy name
...
const fetch = (
  ...
  if (token) headers.set("X-Connect-AI-Token", token);
...
const openWebSocket = (url: string): WebSocket => {
  const token = apiToken();
  return token
    ? new WebSocket(url, ["connect-ai", token])
    : new WebSocket(url);
};
...
// --- Google connect (local helper on port 8766) ---
// Cloud sign-in was removed entirely: Google connectors are connected through the
// local helper — one-click "Sign in with Google" once an OAuth client is saved
// (wizard walks through that, one time), tokens land in google-tokens.json and
// are pushed into the sidecar's connectors.
//
// ONE login covers every Google service: signing in connects Gmail, Google
// Calendar and Google Drive together, and signing out revokes the grant at
// Google and disconnects all three. There is no per-Google-connector token
// paste anywhere in the UI — GOOGLE_CONNECTORS is the list that routes to it.

export const GOOGLE_HELPER_BASE = "http://127.0.0.1:8766";
export const GOOGLE_WIZARD_URL = `${GOOGLE_HELPER_BASE}/google/wizard`;
export const GOOGLE_CONNECTORS = ["gmail", "google_calendar", "google_drive"];

/** One Google login stored on this computer (google-tokens.json). */
export interface GoogleAccount {
  name: string; // slot key — the Google email for logins made through consent
  email: string | null;
  has_refresh_token: boolean;
  expires_in: number | null; // seconds until the access token expires
  is_active: boolean; // the account new connects default to
}

/** What one Google connector currently holds in the sidecar. */
export interface GoogleService {
  connected: boolean;
  accounts: string[]; // emails signed into this service
  title: string;
  default_account?: string;
}

export interface GoogleState {
  has_client: boolean; // an OAuth Desktop client is saved → login is one click
  accounts: GoogleAccount[];
  services: Record<string, GoogleService>; // keyed by connector name
  reachable: boolean; // false when the local helper (8766) isn't answering
}

const GOOGLE_STATE_DOWN: GoogleState = {
  has_client: false,
  accounts: [],
  services: {},
  reachable: false,
...
export function openGoogleWizard() {
  window.open(GOOGLE_WIZARD_URL, "_blank", "noopener");
}

/** Who is signed in with Google, and which Google services that login drives. */
export async function getGoogleState(): Promise<GoogleState> {
  try {
    const res = await fetch(`${GOOGLE_HELPER_BASE}/google/setup-state`);
    if (!res.ok) return GOOGLE_STATE_DOWN;
    const data = (await res.json()) as Partial<GoogleState>;
    return {
    ...
    return GOOGLE_STATE_DOWN; // launcher helper not running
  }
}

/** Re-wire Gmail + Calendar + Drive from the stored login — the repair path for
 * a 401 or for a service that was disconnected by hand. No consent screen. */
export async function googleConnectAll(): Promise<{ ok: boolean; error?: string }> {
  try {
    const res = await fetch(`${GOOGLE_HELPER_BASE}/google/connect-all`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{}",
    });
...
/** Make one Google login the default account for every Google service. */
export async function googleActivate(
  account: string,
): Promise<{ ok: boolean; error?: string }> {
  try {
    const res = await fetch(`${GOOGLE_HELPER_BASE}/google/activate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
/** Sign out of Google: revoke at Google, delete the local token, disconnect
 * Gmail + Calendar + Drive. Omit `account` to sign out of every Google login. */
export async function googleSignOut(
  account?: string,
): Promise<{ ok: boolean; error?: string }> {
  try {
    const res = await fetch(`${GOOGLE_HELPER_BASE}/google/logout`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
/** One-click Google login: ask the helper to open the Google consent screen
 * directly (it opens the browser tab itself). Falls back to the wizard page
 * when there's no OAuth client yet or the helper is unreachable — the wizard
 * walks through the one-time client setup and the Playground alternative. */
export async function startGoogleLogin(): Promise<void> {
  try {
    const res = await fetch(`${GOOGLE_HELPER_BASE}/google/auth-start`);
    const data = await res.json().catch(() => ({}) as { error?: string });
    if (res.ok && !data.error) return; // Google consent tab is already open
  } catch {
    /* helper down — the wizard URL will show the connection error itself */
  }
  openGoogleWizard();
}
...
// bot is a member (Slack API constraint); is_member=false → "invite @AI cowork" hint.
