...
export function AutomationQuickstart({
  ...
  // §30 connect states: "waiting" while the Google wizard is open in the browser —
  // the handoff strip explains the out-of-band finish. Non-Google connectors open
  // the AddConnectionModal instead (manual tokens / local MCP OAuth).
  const [connFlow, setConnFlow] = useState<{ name: string; phase: "opening" | "waiting" } | null>(
    null,
  );
  const [modalConn, setModalConn] = useState<string | null>(null);
  ...
  // is being configured (connects land out-of-band in the browser).
  ...
  const startConnect = (name: string) => {
    if (GOOGLE_CONNECTORS.includes(name)) {
      // Google connectors: one-click login via the local helper (:8766) — the
      // Google consent tab opens; the poll flips the row when the token lands.
      void startGoogleLogin();
      setConnFlow({ name, phase: "waiting" });
      return;
    }
    // Everything else connects in place: manual tokens / local MCP OAuth.
    setModalConn(name);
  };
  ...
  return (
    <div className="mb-4">
              ...
              {picked.needsChannel && (
                <>
                  ...
                  <p className="text-[11px] text-warnInk mt-1">
                    The bot must be a member of the channel — invite the bot in Slack if it isn't.
                  </p>
                </>
              )}
      ...
      {modalConn && connState(modalConn) && (
        <AddConnectionModal
          c={connState(modalConn)!}
