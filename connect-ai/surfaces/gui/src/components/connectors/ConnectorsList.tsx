...
// The list opens with the Google card (connect-AI): Google is one sign-in covering
// Gmail + Calendar + Drive, so it gets one place to sign in and out — the three
// connector rows below are for their tools and settings, not for credentials.
...
export function ConnectorsList({
  ...
  return (
    <div>
      ...
      {!q && <GoogleAccountCard connectors={connectors} onChanged={onChanged} />}

      {connected.length > 0 && (
        <>
          <div className={GRP_H + (q ? " !mt-0" : "")}>Connected · {connected.length}</div>
          <div className={GRP}>
            {connected.map((c) => (
              <button
                key={c.name}
