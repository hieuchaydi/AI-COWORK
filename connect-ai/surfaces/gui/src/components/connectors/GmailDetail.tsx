...
// Adding an account opens the LOCAL Google wizard (launcher helper on :8766) —
// sign in with Google there — one sign-in also connects Calendar and Drive; the
...
export function GmailDetail({ c, slack: _slack, onChanged }: DetailProps) {
  ...
  const addAccount = () => {
    setBusy(true);
    void startGoogleLogin(); // Google consent opens in the browser; the poll picks it up
    ...
    <div data-testid="gmail-detail">
      <div className="flex items-center gap-3.5 mb-5">
        ...
        <button
          ...
          onClick={addAccount}
          disabled={busy}
          title="Sign in with Google in your browser"
        >
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            Sign in with Google — one sign-in also connects Calendar and Drive. Each mailbox
            stays separate, and agents say which one they use. Tokens stay on this computer.
          </div>
        </div>
      )}
      ...
      {accounts.length > 0 && (
        <>
          ...
          <div className={FOOT}>
            These are Google sign-ins — the same login also runs Google Calendar and
            Google Drive. × removes the mailbox from Gmail only; sign out entirely from
            Connectors ▸ Google.
          </div>
        </>
      )}
