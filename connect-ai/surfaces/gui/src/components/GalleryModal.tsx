...
//
// Trust model: the pitch is publisher metadata but the capabilities card is derived
// locally from the manifest by our own parser; installs land disabled pending
// consent under Personas. No sign-in — when the gallery feed is unreachable the
// modal says so and points at the local install paths (folder / Git URL).
...
function sourceOf(p: GalleryPersona): Exclude<Source, "all"> {
  return p.publisher === "AI cowork" ? "openworker" : "team";
}
    ...
    <div data-testid="gallery-cards">
      ...
      {unavailable && (
        <div className="text-[12.5px] text-muted">
          The gallery is unreachable right now — installing personas from a folder or
          Git URL on the Personas page always works.
        </div>
      )}
