...
export function ConnectorTools({ c, onChanged }: { c: Connector; onChanged: () => void }) {
  ...
  return (
    <div className="border-t border-line px-3.5 py-3">
      <div className={SEC_H + " mb-2"}>Tools exposed to Workspace</div>
      <div className="space-y-1.5">
        {c.tools.map((tool) => (
          <label
...
//
// Google connectors never reach the token form: Gmail, Google Calendar and Google
// Drive all run on ONE local Google sign-in (connect-AI), so every place that would
// have asked for an access token routes here into the sign-in button instead.
export function ConnectSetup(props: {
  c: Connector;
  onConnected: () => void;
  manualOnly?: boolean;
}) {
  if (GOOGLE_CONNECTORS.includes(props.c.name)) {
    return <GoogleConnectSetup c={props.c} onConnected={props.onConnected} />;
  }
  return <ManualConnectSetup {...props} />;
}

// The one Google sign-in, rendered wherever a Google connector asks to connect.
// The consent flow finishes in the browser, so poll until the sidecar flips.
function GoogleConnectSetup({ c, onConnected }: { c: Connector; onConnected: () => void }) {
  ...
  return (
    <div className="border-t border-line px-3.5 py-3 space-y-2" data-testid="google-connect-setup">
      <p className="text-[12.5px] text-muted leading-relaxed">
        {c.title} runs on your Google sign-in — one sign-in connects Gmail, Google
        Calendar and Google Drive together. Tokens stay on this computer.
      </p>
      <button
        className={BTN_ACCENT}
        data-testid="google-connect-setup-btn"
        onClick={() => {
        ...
        disabled={waiting}
      >
        {waiting ? "Finish in your browser…" : "Sign in with Google"}
      </button>
    </div>
  );
}

function ManualConnectSetup({
  c,
  onConnected,
  manualOnly = false,
}: {
  ...
  const [busy, setBusy] = useState(false);
  const [waiting, setWaiting] = useState(false); // MCP flow: browser is open
  const [error, setError] = useState<string | null>(null);
  ...
  const mcpOneClick = async () => {
    ...
    // Completion arrives via the poll — the sidecar flips the connector
