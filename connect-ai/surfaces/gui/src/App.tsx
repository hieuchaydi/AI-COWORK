    ...
    const handleEvent = (ev: WsEvent) => {
      ...
      switch (ev.type) {
        ...
        case "assistant_delta":
          // First token after a quota park means the wait paid off — drop the notice.
          setItems((p) =>
            p.some((it) => it.kind === "notice" && it.waiting)
              ? p.filter((it) => !(it.kind === "notice" && it.waiting))
              : p,
          );
        ...
        case "model_failover":
          ...
          if (d.to) setModel(d.to);
          setItems((p) => [
            ...p,
            {
              kind: "notice",
              tone: "info",
              text: `${d.from} is out of quota — continuing on ${d.to}.`,
            },
          ]);
          break;
        case "model_waiting":
          // Every model is walled: the turn is parked, not dead. Replace any
          // previous waiting notice so a long hold doesn't spam the transcript.
          setItems((p) => [
            ...p.filter((it) => !(it.kind === "notice" && it.waiting)),
            {
              kind: "notice",
              ...
              waiting: true,
              text:
                `Out of quota on every model — waiting ${Math.round(d.retry_in)}s, then ` +
                `picking this turn back up. Stop to cancel.`,
            },
          ]);
          break;
        case "interrupted":
  ...
  if (booting || !uiReady) {
    return (
      <div className={"app boot-splash" + (overlay ? " tauri-overlay" : "")}>
        ...
        {overlay && (
          <div className="titlebar-drag" data-tauri-drag-region>
            <span className="titlebar-brand brand-wordmark">
              <Icon name="logo" size={13} className="mark" /> Workspace
            </span>
          </div>
        )}
        {simOverlay && (
        ...
        {/* The real AI cowork mark (6-point star, same as the app/tray icon) — the old
            ✦ text glyph was a 4-point sparkle that read as another product's logo. */}
        <div className="boot-mark">
          <Icon name="logo" size={38} />
        </div>
        <div className="boot-text">
          {resumedExisting ? "Restoring your session…" : "Starting Workspace…"}
