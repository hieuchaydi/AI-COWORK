...
class PermissionEngine:
    ...
    def evaluate(
        ...
        if tool_name in self.task_rules:
            ...
            if target:
                allowed = self.task_rules[tool_name]
                # 1) exact string match (original semantics)
                # 2) tolerate the "platform:id" vs "id" mismatch that happens when the
                #    LLM proposed a grant with or without the connector prefix. Match
                #    when the incoming target's last colon-segment equals a granted
                #    value, OR when a granted "platform:id" is exactly the target.
                incoming_tail = target.rsplit(":", 1)[-1]
                incoming_variants = {target, incoming_tail}
                matched = None
                for grant in allowed:
                    grant_tail = grant.rsplit(":", 1)[-1]
                    if grant in incoming_variants or grant_tail == incoming_tail:
                        matched = grant
                        break
                if matched:
                    rule = f"{tool_name} → {target} (via grant '{matched}')"
