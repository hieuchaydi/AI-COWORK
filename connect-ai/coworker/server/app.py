...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    # connect-AI branding: the new env name wins, the legacy one still works so an
    # older .env (or a shell that exported it) doesn't lock the user out.
    ...
    def _request_authenticated(request: Request) -> bool:
        # Both header spellings are accepted: `x-connect-ai-token` is what this
        # project sends now, `x-openworker-token` keeps older clients (scripts,
        # bridges, a stale GUI bundle) working across the rename.
        provided = request.headers.get("x-connect-ai-token") or request.headers.get(
            "x-openworker-token", ""
        )
        return bool(
            api_token
    ...
    def _accept_subprotocol(ws: WebSocket) -> "str | None":
        """Echo back the label the client actually offered. A WebSocket handshake
        fails unless the server selects one of the client's protocols, so hard-coding
        one name would break every client using the other spelling — which is exactly
        what the connect-AI rename would have done to older GUI bundles."""
        if not api_token:
            return None
        offered = [
            part.strip()
        ...
        for label in ("connect-ai", "openworker"):
            if label in offered:
                return label
        return "connect-ai"
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid connect-AI API token"},
            status_code=401,
        )
