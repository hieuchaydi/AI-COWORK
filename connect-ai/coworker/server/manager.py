...
class SessionManager:
    ...
    def get_engine(
        ...
        *,
        ...
        engine = build_engine(
            ...
            # Quota wall → next configured model instead of a dead turn.
            fallback_models=self.usable_models,
            roots=roots,
    ...
    def list_artifacts(self, session_id: str) -> list[dict[str, Any]]:
        ...
        # (root, path-prefix) pairs. The workspace comes first so it wins on name
        # collisions, matching _artifact_target's resolution order.
        roots: list[tuple[Path, str]] = []
        if workspace:
            ws = Path(workspace).expanduser().resolve()
            if ws.is_dir():
                roots.append((ws, ""))
        out_root = _unified_output_root()
        if out_root and not any(_within(out_root, r) for r, _ in roots):
            roots.append((out_root, "outputs/"))
        if not roots:
            return []
        ...
        for root, prefix in roots:
            for path in root.rglob("*"):
                try:
                    ...
                    out.append(
                        {
                            "path": prefix + rel.as_posix() if prefix else str(rel),
    ...
    def _artifact_target(
        ...
        """Resolve an artifact path under the session's workspace (or the shared
        outputs tree), or (None, error)."""
        record = self.session_store.load(session_id)
        workspace = record.workspace if record else self.default_workspace
        escaped = False
        if workspace:
            ...
            if not _within(target, root):
                escaped = True  # may still be a legit file in the outputs tree below
            elif target.is_file():
                return target, None
        # Local patch: save_csv & friends write to <project>/outputs, not the
        # workspace, so "outputs/csv/x.csv" (what the model links as an artifact:
        # chip), the bare "csv/x.csv", and the absolute path the tool echoes back
        # all have to resolve here. See _unified_output_root().
        out_root = _unified_output_root()
        if out_root:
            rel = path.replace("\\", "/").strip().lstrip("/")
            tails = [rel]
            if rel.startswith("outputs/"):
                tails.append(rel[len("outputs/") :])
            for tail in tails:
                cand = (out_root / tail).expanduser().resolve()
                if _within(cand, out_root) and cand.is_file():
                    return cand, None
            given = Path(path).expanduser()
            if given.is_absolute():
                given = given.resolve()
                if _within(given, out_root) and given.is_file():
                    return given, None
        if not workspace:
    ...
    def usable_models(self) -> list[str]:
        ...
        Same rule the composer's selector uses (`get_settings`), exposed on its own
        so the turn engine can fail over to the next one when a model hits its quota
        instead of ending the turn (connect-AI patch).
        """
        ...
        return [m for m in self._curated_models() if _selectable(m)]
...
def _within(child: Path, parent: Path) -> bool:
    """True when `child` sits inside (or is) `parent`. Both must be resolved."""
    try:
...
def _unified_output_root() -> Optional[Path]:
    """The shared <project>/outputs tree, or None when it doesn't exist.

    Local patch: save_csv / save_artifact / browser_screenshot / download_file write
    every user-visible file into ONE folder at the project root (served by the
    launcher helper on 8766), NOT into the per-session workspace. The artifact rail
    and `artifact:` chips are workspace-scoped, so without this they resolve to
    "not found" for anything those tools produce. Mirrors crawl._output_root().
    """
    env = os.environ.get("COWORKER_OUTPUT_DIR")
    if env:
        base = Path(env).expanduser()
    else:
        # manager.py sits at <root>/connect-ai/coworker/server/, so parents[3] = root
