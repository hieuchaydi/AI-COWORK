...
# is most turns. Instead the engine fails over to the next configured model and, if
# every model is exhausted, waits for the limit to lift and picks the turn back up.
# Waits are cancellable (Stop) and bounded by QUOTA_WAIT_BUDGET_SEC.
_QUOTA_BACKOFF_SEC = (20, 45, 90, 180, 300)  # then 300s until the budget runs out


def _wait_budget() -> float:
    """Total seconds to keep a turn parked waiting for quota. 0 disables waiting
    (the turn errors out immediately, the pre-patch behaviour)."""
    try:
        return float(os.environ.get("COWORKER_QUOTA_WAIT_SECONDS", "900"))
    except ValueError:
        return 900.0
...
class TurnEngine:
    def __init__(
        ...
        *,
        ...
        # Returns the models this account can actually use, best-first (the picker's
        # order). Consulted only when the current model hits a quota/access wall —
        # see `_recover_from_model_error`. None → no failover, only waiting.
        fallback_models: Optional[Callable[[], list[str]]] = None,
    ) -> None:
        ...
        # Models this turn already burned on a quota/access wall — never retried
        # until the turn ends (a second attempt just costs another round-trip).
    ...
    async def _recover_from_model_error(
        self, exc: Exception
    ) -> AsyncIterator[Optional[Event]]:
        """Try to keep the turn alive after a quota / rate-limit / no-access failure.
        ...
        if kind in (QUOTA, NO_ACCESS):
            ...
            if nxt:
                ...
                yield Event(
                    ...
                    {"from": failed, "to": nxt, "reason": reason or str(exc)},
                )
                yield None
                return
        if kind == NO_ACCESS:
            return  # waiting cannot grant access — surface it

        # Out of models: hold the turn until the limit lifts, then pick it back up.
        async for event in self._wait_for_capacity(exc):
            yield event

    async def _wait_for_capacity(
        self, exc: Exception
    ) -> AsyncIterator[Optional[Event]]:
        """Park the turn while every model is over its limit, retrying on a backoff
        until the budget runs out. Stop cancels the wait immediately."""
        budget = _wait_budget()
        if budget <= 0 or self._quota_waited >= budget:
            return
        delay = _QUOTA_BACKOFF_SEC[
            min(self._quota_attempts, len(_QUOTA_BACKOFF_SEC) - 1)
        ]
        delay = min(float(delay), budget - self._quota_waited)
        if delay <= 0:
            return
        yield Event(
            EventType.MODEL_WAITING,
            {
                "model": self.model,
                "retry_in": delay,
                "waited": self._quota_waited,
                "budget": budget,
        ...
        # failure re-explore the full list instead of staying on one exhausted id.
    ...
    async def _loop(self) -> AsyncIterator[Event]:
        iterations = 0
        # Per-turn quota recovery state (failover memory + how long we've parked).
        self._exhausted_models = set()
        self._quota_waited = 0.0
        self._quota_attempts = 0
        while True:
            ...
            try:
                ...
                # Quota/rate-limit walls are recoverable: switch models, or park the
                # turn until the limit lifts. Only give up when neither can help.
                # Nothing was streamed on these (the call fails before the first
                # token), so the transcript stays clean across the retry.
                if not (streamed or streamed_reasoning):
                    recovered = False
                    async for event in self._recover_from_model_error(exc):
                        if event is None:  # sentinel: recovery succeeded, retry now
                            recovered = True
                            break
                        yield event
                    if recovered:
                        iterations -= 1  # a failover must not eat the turn's budget
                        continue
                    if self._cancel.is_set():
                        # Stopped while parked on a quota wait — that's an
                        # interrupt, not a provider error.
