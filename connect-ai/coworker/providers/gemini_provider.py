...
# Gemini 3 hard-rejects (400 INVALID_ARGUMENT) any function_call replayed without a
# thought_signature. That happens whenever a tool call in the history did NOT come from
# Gemini 3 with a captured signature: a quota failover produced it on another model, an
# older turn predates signature capture, or Gemini's own parallel-call signing skipped
# it (a known preview quirk). Google publishes a sentinel — set it as the signature and
# the API skips validation for that part instead of failing the whole turn. Sent as the
# same base64 string form as real signatures (SDK decodes with val_json_bytes="base64").
_SKIP_SIGNATURE = base64.b64encode(b"skip_thought_signature_validator").decode("ascii")
    ...
    for message in messages[index:]:
        ...
        if role == "system":
            ...
            for i, call in enumerate(message.get("tool_calls") or []):
                ...
                part["thought_signature"] = sig or _SKIP_SIGNATURE
                parts.append(part)
            if parts:
