...
from .tools.clock import make_current_time_tool
...
def build_engine(
    *,
    ...
    # Models to fail over to when this one hits its quota (connect-AI patch).
    # The manager passes its live picker list; None → no failover.
    ...
    # current_time: fresh wall-clock the LLM can call before scheduling / quoting time.
    # Complements the per-turn <current-time> block in the system context — tiny models
    # sometimes ignore prompt injections, but they consistently follow tool contracts.
    registry.register(make_current_time_tool())
    # Browser (Playwright, persistent Chromium — login state survives restarts).
    # 13 tools: open / read / read_selector / click / fill / press / screenshot /
    # wait_for / evaluate / get_links / get_forms / current_url / close. Bypasses
    # MCP layer so it's always available regardless of MCP loading. Connector-family
    # personas respect the Browser card's per-tool toggles (tool_defs.py names);
    # personas without connector settings keep the full set.
    browser_tools = make_browser_tools()
    if enabled_tools is not None:
        browser_tools = [t for t in browser_tools if t.__name__ in enabled_tools]
    registry.register_all(browser_tools)
    # Crawl / scrape (httpx + BeautifulSoup). Bulk primitives — one call replaces
    # a many-turn agent loop for static-HTML sites:
    ...
    # Custom tools: auto-discover from custom_tools/ in project/workspace + create_custom_tool
    from .tools.custom_loader import discover_custom_tools, make_create_custom_tool
    ...
    def context_provider() -> str:
        parts = []
        # Fresh wall-clock every turn — the static `Current local time` in the
        # session-start environment block goes stale as chats stretch across
        # hours; injecting the live time here fixes agents saying "hiện tại là
        # 13:07" when it's actually 22:07. Default UTC+7 (Asia/Ho_Chi_Minh);
        # override via env TZ_OFFSET_HOURS / TZ_LABEL.
        import os as _os
        from datetime import datetime as _dt, timedelta as _td, timezone as _tz
        try:
            _off = float(_os.environ.get("TZ_OFFSET_HOURS", "7"))
        except ValueError:
            _off = 7.0
        _label = _os.environ.get("TZ_LABEL", "UTC+7 Asia/Ho_Chi_Minh")
        _now = _dt.now(_tz(_td(hours=_off)))
        parts.append(
            "<current-time>\n"
            f"Now: {_now.strftime('%Y-%m-%d %H:%M:%S')} {_label} ({_now.strftime('%A')})\n"
            f"Epoch: {int(_now.timestamp())}\n"
            "AUTHORITATIVE — use this for every time calculation, scheduling, "
            "and 'what time is it' question. Ignore any older time in the "
            "system prompt if it disagrees.\n"
            "</current-time>"
        )
