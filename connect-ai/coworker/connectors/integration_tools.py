...
def make_integration_tools(
    ...
    *,
    ...
    # Browser tools live in coworker/tools/browser.py (persistent Chromium) and are
    # registered by agent.py — the legacy browser_automation set is retired so only
    # ONE Chromium/toolset exists.
    ...
    def github_list_my_repos(
        ...
        """List repositories the AUTHENTICATED user has access to, including
        PRIVATE. Hits /user/repos which requires the PAT — without it, only
        public repos are visible. `visibility` ∈ all|public|private."""
        out = _github_call(
            secrets,
            "GET",
            "/user/repos",
            params={
                ...
                "sort": sort if sort in ("created", "updated", "pushed", "full_name") else "updated",
        ...
        if "error" in out:
            return out
        repos = out["data"] if isinstance(out["data"], list) else []
        slim = [
            {
        ...
        return {
            ...
            "per_page": per_page,
            "hint": (
                "Nếu count == per_page thì có thể còn trang tiếp theo — tăng `page`. "
                "Đặt visibility='private' để chỉ lấy private repos."
            ),
            "repos": slim,
        }

    github_list_my_repos.__name__ = "github_list_my_repos"
    tools.append(
        _attach(
            github_list_my_repos,
            _schema(
                "github_list_my_repos",
                "List repos of the AUTHENTICATED user (public + private + org + "
                "collaborator). Requires connected GitHub PAT — call this instead "
                "of web_fetch on github.com/<username> which only sees public.",
                {
                    "visibility": {"type": "string", "description": "all|public|private"},
    ...
    def github_list_user_repos(
        username: str, per_page: int = 100, page: int = 1
    ) -> dict[str, Any]:
        """List PUBLIC repos of ANOTHER user or organization. Uses /users/{name}/
        repos (or /orgs/{name}/repos if the entity is an org). For your OWN
        private repos, use `github_list_my_repos` instead."""
        out = _github_call(
            secrets,
            "GET",
    ...
    github_list_user_repos.__name__ = "github_list_user_repos"
    tools.append(
        _attach(
            github_list_user_repos,
            _schema(
                "github_list_user_repos",
                "List PUBLIC repos of a specific user or organization.",
                {
