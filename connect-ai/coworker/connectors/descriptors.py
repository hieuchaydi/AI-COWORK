...
# connect-AI patch: the three Google connectors run on ONE local Google sign-in
# (google_auth.py pushes the same access_token into all of them), so each needs a
# whoami that (a) proves the token carries that service's scope and (b) returns
# the Google email — the account key every Google connector is filed under.
def _validate_gmail(creds: dict) -> ValidationResult:
    return _validate_whoami(
        "GET",
        "https://gmail.googleapis.com/gmail/v1/users/me/profile",
        headers={"Authorization": f"Bearer {creds.get('access_token', '')}"},
        identity=lambda d: d["emailAddress"],
    )


def _validate_google_calendar(creds: dict) -> ValidationResult:
    # The primary calendar's id IS the account's email.
    return _validate_whoami(
        "GET",
        "https://www.googleapis.com/calendar/v3/calendars/primary",
        headers={"Authorization": f"Bearer {creds.get('access_token', '')}"},
        identity=lambda d: d["id"],
    )
...
DESCRIPTORS: list[ConnectorDescriptor] = [
    ...
    ConnectorDescriptor(
        ...
        fields=[
            Field(
                ...
                "OAuth access token",
                secret=True,
                help="Filled in by the local Google sign-in — no need to paste anything.",
            ),
        ],
        instructions=[
            "Sign in with Google once — the same login connects Gmail, Google Calendar and Google Drive together.",
            "Tokens stay on this computer and refresh themselves every 50 minutes.",
        ],
        validate=_validate_gmail,
        available=True,
        managed=True,
    ...
    ConnectorDescriptor(
        ...
        validate=_validate_google_calendar,
        available=True,
        managed=True,
