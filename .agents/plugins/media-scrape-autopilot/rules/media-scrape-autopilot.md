...
Use these rules for any crawl, scrape, product review, catalog, gallery, article library, or listing task that can include image or video URLs.
...
- Use the local ingest queue when configured, or ask for login only once if credentials/session are missing.
